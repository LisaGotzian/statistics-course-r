...
wilcox.test(August, November, paired=T, alternative = "less")
mean(August)
mean(November)
...
hist(cars$dist) # not normally distributed
hist(cars$speed)
shapiro.test(cars$dist)
shapiro.test(cars$speed)
cor.test(cars$dist, cars$speed) #cannot report it


# 3a) linear model because we have 2 continuous variables
...
hist(resid(logModel)) # we did do better!
